...
/**
 * Mock Mongoose Model object for Product.
...
const mockProductModel = {
    ...
    findById: jest.fn(),   // Mock findById method here
    deleteOne: jest.fn()    // Mock deleteOne method here
};

/**
 * Mock DB object that simulates the mongoose db interface.
 * In real code, we use db.model('Product') to get the Product model.
 * Here, we return our mockProductModel whenever model() is called.
 */
const mockDb = {
    model: jest.fn().mockReturnValue(mockProductModel),  // Return mockProductModel when model is called
    Product: mockProductModel  // Explicitly define Product here for use in tests
};
